...
def main():
    ...
    prompt2 = '''输出图中三轮车目标颜色,从以下颜色列表中选择一个最可能的颜色，如果是晚上或者比较模糊的情况，仔细分辨可能的颜色
    ...
    prompt3 = '''输出图中三轮车目标颜色,从以下颜色中选择一个最可能的颜色：
    {
    "车身颜色": [
        "不确定",
        "红色",
    ...
    输出格式为一行json，为一个map,key为颜色名称，value为对应的置信度。不要输出markdown，即不输出\'\'\'json \'\'\', 仅输出json。
    输出示例如下，包含颜色与对应置信度，按照置信度从高到底排列，输出前三个最可能的颜色：
    {'灰色':置信度, '白色':置信度, '黑色':置信度}

    要求输出最确定准确的颜色，有的图可能受光照、遮挡、清晰度等影响，如果不能确定就输出'不确定'，要求结果准确率最高。
    
    '''

    prompt4 = '''输出图中三轮车目标颜色,从以下颜色列表中选择一个最可能的颜色，如果是晚上或者比较模糊的情况，仔细分辨可能的颜色
    ...
    prompt5 = '''输出图中三轮车目标颜色,从以下颜色中选择一个最可能的颜色：
    {
    "车身颜色": [
        "不确定",
        "红色",
    ...
    输出格式为一行json，为一个map,key为颜色名称，value为对应的置信度。不要输出markdown，即不输出\'\'\'json \'\'\', 仅输出json。
    输出示例如下，包含颜色与对应置信度，按照置信度从高到底排列，输出前三个最可能的颜色，例如：
    {'灰色':置信度, '白色':置信度, '黑色':置信度}
    置信度为0.00-1.00之间的数字，表示当前颜色的可能性
    仅输出颜色和置信度，输出三个最可能的颜色

    要求输出最确定准确的颜色，关注图片中最主要车辆且占车身面积最大部分的颜色，有的图可能受光照、遮挡、清晰度等影响，如果不能确定就输出'不确定'，要求结果准确率最高。
    
    '''

    prompt6 = '''输出图中三轮车目标颜色,从以下颜色列表中选择一个最可能的颜色，如果是晚上或者比较模糊的情况，仔细分辨可能的颜色
